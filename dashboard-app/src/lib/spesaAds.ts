// La spesa Ads dal foglio "Report Storico": una riga per giorno e campagna.
//
// Ogni mese vive in un tab a parte, con nome "Report_Storico_YYYY-MM", e la
// lettura e' la stessa per chiunque la chieda - la tabella per il periodo
// scelto, il grafico dell'andamento per gli ultimi dodici mesi. Stava dentro
// l'endpoint della tabella; copiarla nel secondo avrebbe voluto dire copiare
// anche il riconoscimento dei tab inesistenti, che e' la parte che conta.
//
...
export function monthsBetween(fromIso: string, toIso: string): Array<{ year: number; month: number }> {
  const now = new Date();
...
/** Tutte le righe di spesa dei mesi toccati dall'intervallo, tab per tab. */
export async function leggiSpesaAds(fromIso: string, toIso: string): Promise<CampaignAdsSpendRow[]> {
    ...
    months.map(async ({ year, month }) => {
      ...
      try {
        ...
        if (predefinito !== null && testo === predefinito) {
          console.warn(`[spesa-ads] ${tabName}: tab inesistente (ricevuto il foglio predefinito), ignorato`);
          return;
        }
        ...
        if (scartate) {
          console.warn(`[spesa-ads] ${tabName}: scartate ${scartate} righe fuori dal mese`);
        }
      } catch (err) {
        // Tab del mese non ancora disponibile (o non ancora creato): si ignora.
        console.warn(`[spesa-ads] tab non disponibile: ${tabName}`, err instanceof Error ? err.message : err);
      }
    })
  );
